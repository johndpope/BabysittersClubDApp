...
contract Club {
    ...
    constructor(string _symbol, string _name, address _token, address _memberAddr, string _memberName, uint _tokensForNewMembers) public {
        members.init();
        symbol = _symbol;
        name = _name;
...
contract ClubFactory is Owned {
    ...
    event ClubListing(address indexed clubAddress, string clubSymbol, string clubName,
    ...
    ) public returns (Club clubAddress, ClubToken tokenAddress) {
        tokenAddress = new ClubToken(tokenSymbol, tokenName, tokenDecimals);
        _verify[tokenAddress] = true;
        deployedTokens.push(tokenAddress);
        clubAddress = new Club(clubSymbol, clubName, tokenAddress, msg.sender, memberName, tokensForNewMembers);
        _verify[clubAddress] = true;
        deployedClubs.push(clubAddress);
        emit ClubListing(clubAddress, clubSymbol, clubName, tokenAddress, tokenSymbol, tokenName, tokenDecimals, msg.sender, tokensForNewMembers);
    }
